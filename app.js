// ============================================================
//  app.js — Gastei!
//  Supabase como backend, funciona offline com localStorage fallback
// ============================================================

import { createClient } from 'https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm'

// ── CONFIG — altere aqui com suas credenciais do Supabase ──
...
// ============================================================
//  DADOS FIXOS — ajuste conforme necessidade
// ============================================================
...
// Mês financeiro: começa dia 10 e vai até dia 9 do mês seguinte.
// Ex: 10/jun → 09/jul = mês financeiro "2025-06"
//     01/jun → 09/jun ainda é mês financeiro "2025-05"
const mesFinanceiro = () => {
  const d = new Date()
  const dia = d.getDate()
  // Antes do dia 10: ainda pertence ao mês anterior
  const ref = dia < 10 ? new Date(d.getFullYear(), d.getMonth() - 1, 1) : d
  return `${ref.getFullYear()}-${String(ref.getMonth() + 1).padStart(2, '0')}`
}

const mesAtual = mesFinanceiro  // alias usado no resto do código
...
function goPage(p) {
  ...
  document.querySelectorAll('.nav-btn')[['dash', 'lancar', 'fixos', 'historico'].indexOf(p)].classList.add('active')
...
function atualizarBotao() {
  ...
  btn.className = 'btn-primary ' + (pronto ? (pessoaSel === 'fred' ? 'btn-fred' : 'btn-mya') : 'btn-disabled')
}

function renderLancar() {
  ;['fred', 'mya'].forEach(p => {
    document.getElementById('pbtn-' + p).classList.toggle('active', pessoaSel === p)
    const g = gastos.filter(x => x.pessoa === p).reduce((a, b) => a + b.valor, 0)
    document.getElementById('pgasto-' + p).textContent = fmt(g)
  })
  const inp = document.getElementById('inp-valor')
  inp.className = 'valor-input' + (pessoaSel ? ' ' + pessoaSel : '')
  renderCatGrid()
  showAlerta()
  atualizarBotao()
}
...
function renderDash() {
  ...
  CATS.forEach(c => {
    ...
    const sub = (gF > 0 || gM > 0) ? `<span style="font-size:11px;color:var(--color-text-secondary)">${gF > 0 ? 'F:' + fmt(gF) : ''}${gF > 0 && gM > 0 ? ' · ' : ''}${gM > 0 ? 'M:' + fmt(gM) : ''}</span>` : ''
...
async function init() {
  ...
  document.getElementById('header-sub').textContent = 'Controle financeiro do casal'
